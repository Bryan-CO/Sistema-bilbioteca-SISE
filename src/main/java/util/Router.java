...
public class Router {
	...
	private String formatRoute() {
		...
		return (reqRoute.length() >= 2 && reqRoute.endsWith("/")) ? reqRoute.substring(0, reqRoute.length() - 1)
				: reqRoute;
	}
	...
	private void removeParametersFromRoutes(List<String> subReqRoute, List<String> subRuta,
			Map<Integer, String> indexElementMap) {
		List<Integer> keys = new ArrayList<>(indexElementMap.keySet());

		// Ordeno de mayor a menor para evitar errores de indices inexistentes al
		// eliminar
